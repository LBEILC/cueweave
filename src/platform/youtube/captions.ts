...
export async function fetchCaptionTrackViaInnertube(options: {
  ...
  const selected = selectInnertubeTrack(rawTracks, options.preferredLanguageCode);
  if (!selected?.baseUrl || !selected.languageCode) {
    throw new Error('YouTube 播放器接口没有返回可用字幕轨。');
  }

  return fetchCaptionTrack(
    {
      baseUrl: selected.baseUrl,
      languageCode: selected.languageCode,
      name: innertubeText(selected.name) || selected.languageCode,
