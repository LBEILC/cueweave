...
翻译算法、提示词、三档预算和通用校验由共享核心维护；平台负责输入适配、调度、持久化与展示。已实现能力、固定 cue/语义重组的区分以及待实现任务契约以[共享翻译核心](TRANSLATION-CORE.md)为准。下面的扩展流程不能直接视为桌面实现。

```mermaid
flowchart LR
    E[浏览器扩展适配与跟播] --> C[共享翻译核心]
    D[桌面适配与项目任务] --> C
    V[离线评测] --> C
    C --> R[宿主注入的 Provider 访问]
```

图示为职责与依赖目标；当前桌面开发路径已共享传输，完整翻译策略仍待收敛。
...
中文优先在播放器可用宽度内保持单行，只有实际接近播放器边缘时才自然换行。用于分句的逗号、句号、分号和冒号不进入最终中文字幕；内容点号的保留规则见[字幕显示](PRODUCT_SPEC.md#字幕显示)。顿号保留，问号和感叹号用于保留语气，单个空格可以表达口语停顿但不构成 unit 边界。如果一个 unit 内出现两侧均可独立阅读的分句标点，本地校验会记录该 unit，并把它与左右相邻 unit 作为一个连续范围交给模型重新分组，使模型可以把错误归入前一句的词元移动到后一句。重叠的问题范围先合并，再与范围外的合格 unit 拼回完整结果。修复后的较小 unit 仍包含可独立分句的标点时，继续只处理包含其相邻 unit 的问题范围，最多执行三轮定向修复。定向修复已给出准确词元范围但仍残留分句标点时，本地只移除标点并使用模型给出的范围，不根据标点创建或猜测新的时间边界。过短片段只移除标点并保持原 unit。

单条译文明显过长时，本地结合英文词元数量和内部连接结构判断是否存在“值得复审”的强语义信号。连接词和停顿只用于触发复审，不直接成为切点；两侧必须各有足够词元，避免把短并列词组误判为显示边界。命中后只把该 unit 交给现有定向边界修复，要求模型在精确词元范围内生成至少两个显示 unit；完整英文语法句允许由多个 `sentenceEnd: false` 的中间 unit 组成。没有强信号的长 unit 仍走一次普通语义复审。定向修复最多三轮，最终降级解析会关闭长句风险门，保留可用译文而不是因为无法拆分就丢弃翻译。

模型返回的每个 unit 都必须映射到准确的连续英文词元，本地不得按译文长度比例猜测原文时间。解析失败时先剥离常见 Markdown 代码围栏；普通结构或覆盖校验失败时，把无效结果和具体错误交给模型执行一次完整修复；已定位的语义分界错误执行有限次定向修复。修复结果无法满足结构或覆盖要求时使用本地显示字幕；翻译为空或 Provider 故障时保留原文。任何失败都不能阻塞视频播放。
