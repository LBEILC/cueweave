...
调度器将一个连续词元窗口及受限的视频标题、频道、最近字幕和视频术语交给模型。模型返回窗口内的起止索引、翻译、句末标记、候选转录修正和术语；返回值必须符合 JSON Schema，并满足：
...
自动发现的双语术语按视频 ID 保存在 `chrome.storage.local`，经过清洗、去重和容量限制后供后续窗口使用。术语记忆不直接改变既有窗口的缓存身份，避免每新增一个术语就使整段已验证缓存失效；新翻译窗口会使用当前术语集合。
