...
中文优先在播放器可用宽度内保持单行，只有实际接近播放器边缘时才自然换行。逗号、句号、分号和冒号只作为分句信号，不进入最终中文字幕；顿号保留，问号和感叹号用于保留语气。如果一个 unit 内出现两侧均可独立阅读的分句标点，本地校验会要求模型重试并返回多个语义准确的连续词元范围；过短片段只移除标点并保持原 unit。没有自然边界的长表达保持完整，本地不得按字符数拆分译文、拆开英文词或按译文长度比例猜测原文时间。解析失败时先剥离常见 Markdown 代码围栏；结构、覆盖或语义分界校验失败时，把无效结果和具体错误交给模型执行一次严格修复。第二次失败后使用本地显示字幕；翻译为空或 Provider 故障时保留原文。任何失败都不能阻塞视频播放。
