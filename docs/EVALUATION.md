...
统一百分制标准、等级锚点、严重错误门槛和匿名评审流程见[翻译核心评分标准 v1](TRANSLATION-SCORING.md)。新工具不会给未评审模板填分，旧评审也不会自动变成新标准分数。

首轮三位子代理对同批三档输出的评分、实质错误与规则复核，见[2026-09-05 代理匿名评审](AGENT-BLIND-REVIEW-2026-09-05.md)。这是 development 输出的代理基线，未经过人工校准，不代表新视频或多轮翻译稳定性。

本文是[共享翻译核心](TRANSLATION-CORE.md)的评测入口，服务浏览器扩展和桌面端的共同算法与提示词演进。既有实验主要覆盖英中词元重组，不能据此代表固定 cue 路径、多语言或两端实时体验。后续报告须标明核心版本、实际入口、分段方式、语言组合、档位及评审来源；产品内复核与离线评分是两个独立环节。
