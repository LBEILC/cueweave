...
算法、提示词和通用质量规则归共享核心，浏览器扩展与桌面应用都是其宿主；评测工具也是核心调用方。两端输入约束不同，核心需要同时支持固定 cue 与有时间证据的语义重组，不能将共享能力等同于复制扩展的整条流程。当前接入差距、目标任务契约和迁移条件见[共享翻译核心](TRANSLATION-CORE.md)。

桌面开发中的 `services/cue-translation.ts` 目前复用核心传输，但仍有宿主内的提示词和分窗逻辑；这是迁移中的状态，不是长期职责边界，也不是共享三档已经在桌面完成验收的证据。
