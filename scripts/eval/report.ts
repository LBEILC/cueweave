...
export async function renderReport(left: EvalRun, right?: EvalRun): Promise<string> {
  ...
  const rows = groups
    .map((group) => {
      ...
      return `<article id="${group.id}" class="group" data-start="${group.startMs}" data-end="${group.endMs}" data-different="${group.different}" data-issue="${!!warnings}">
  ...
  return `<!doctype html>
      ...
      <p class="muted">记录包含字幕、上下文与模型输出，不包含认证请求头。分享前请检查内容。异常退出的未完成请求见 requests 目录。</p>${renderLogs(left, right ? 'A/' : '', 'A')}${right ? renderLogs(right, 'B/', 'B') : ''}</details>
