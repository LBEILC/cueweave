...
Electron 桌面应用工作区。当前 D0 提供安全宿主、统一的 DOM 播放器、视频链接播放、后台媒体探测、SQLite 基础设施和 Windows x64 安装包。链接入口支持 HTTP(S) 媒体直链，以及公开的 YouTube 和哔哩哔哩单视频；项目与翻译按后续阶段接入，ASR 路线确定前不下载模型或调用识别服务。
...
桌面工作台采用固定窗口布局：视频适应剩余空间，底部播放控制保持可见；字幕面板可收起，支持加载字幕、搜索、当前句标记和点击定位。窄窗口中侧栏覆盖画面右侧，播放控制仍可操作。打开其他视频或关于时，当前播放器持续挂载；取消或返回不会重置播放位置。字幕列表目前只读，项目保存与编辑仍属于 D1。
