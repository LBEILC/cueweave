...
describe('AI subtitle output', () => {
  ...
  it('prefers a grounded full-name correction over an overlapping video alias', () => {
    const tokens = tokensFor('Look at GBT 5.6 soul and GPT6 Astra.');
    const result = parseAiSubtitleOutput(
      JSON.stringify({
        units: [
          {
            startIndex: 0,
            endIndex: 4,
            translation: '具体来看 GPT-5.6 Soul',
            sentenceEnd: false,
          },
          {
            startIndex: 5,
            endIndex: tokens.length - 1,
            translation: '而 GPT-6 Astra 则不同',
            sentenceEnd: true,
          },
        ],
        corrections: [
      ...
      true,
      {
        transcriptEvidence: ['GPT 5.6 Soul'],
        terminology: [{ source: 'GPT-6', translation: 'GPT-6' }],
        entityAliases: [{ source: 'GBT', translation: 'GPT-6' }],
      },
    );

    expect(result.map((cue) => cue.sourceText)).toEqual([
      'Look at GPT-5.6 Soul',
      'and GPT6 Astra.',
    ]);
    expect(result.map((cue) => cue.translation)).toEqual([
      '具体来看 GPT-5.6 Soul',
