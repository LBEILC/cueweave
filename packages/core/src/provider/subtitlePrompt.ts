...
export function buildRevisionPrompt(
  ...
  return [
    ...
    '只返回 edits 数组。没有具体错误就返回空数组，不抄回正确条目。每项必须包含 id、sourceQuote（该条 source 中逐字连续引用）、problem（具体错漏及修正方向）和 translation（修正后的该条完整译文）。ID 只能来自输入且不能重复；原文范围固定，不纠词、不分段。',
    '先独立读 source，确定其主体、动作、受事、修饰维度、逻辑关系与限制，再与草稿逐项核对。优先找含义错误，不以流畅或熟悉为正确依据。比较词必须保留比较的属性；目的、手段、原因和结果不能倒置；反身代词要区分动作对象与受益者；被解释的词应按当前语境而非孤立词义翻译。检查后半句、并列动作、否定、条件、数字、可能性与程度是否遗漏。',
    ...
    '逐对核对相邻译文是否重复同一含义：原文没有重复就不应重复。若草稿错位，从各自 source 重新翻译，不沿用错误草稿的切分逻辑。正确译文逐字保留，不为了偏好替换近义词或增添语气词。只在能指出具体错漏、歧义或妨碍理解的语病时提交 edit；修复该问题时保留其余正确内容。',
